...
def sort_sum(row_cen,row_corn):
    sum_listCen = []
    #sum_listCorn = []
    num_ele = len(row_cen)
    
    for s in range(num_ele):
        sum_cen = row_cen[s].sum()
        #print(row_cen[s],":",sum_cen)
        sum_listCen.append(sum_cen)
        #sum_corn = row_corn[s].sum()
        #sum_listCorn.append(sum_corn)
        
    row_cen = np.array(row_cen,dtype="int64")
    row_corn = np.array(row_corn,dtype="int64")
    
    sum_sortCen = sorted(sum_listCen)
    #sum_sortCorn = sorted(sum_listCorn)
    
    #for new row which is sorted
    rowcen_new = row_cen.copy() 
    rowcorn_new = row_corn.copy()
    
    for i in range(num_ele):
        sum_eleCen = row_cen[i].sum() # sum element of old list
        if sum_eleCen in sum_listCen: # check sum old element with new sort sum
            new_index = sum_sortCen.index(sum_eleCen) #get new sort index by using sum old list to sum_sortList
            #print("old index:",i,"sum_row:",sum_eleCen,"---> new index:",new_index)
            rowcen_new[new_index] = row_cen[i] 
            rowcorn_new[new_index] = row_corn[i]
        else:
            print(i,"not exist")
            
    return rowcen_new,rowcorn_new
