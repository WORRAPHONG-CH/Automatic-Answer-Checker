...
def warp_answerBig(warp_img,corner_list,w,h):
    pts1 = np.float32([corner_list[0],corner_list[1],corner_list[2],corner_list[3]])
    #pts2 = np.float32([[0,900],[0,0],[800,0],[800,900]])
    #pts2 = np.float32([[0,h],[0,0],[w,0],[w,h]])#normal
    #pts2 = np.float32([[w,0],[0,0],[0,h],[w,h]]) #Key01
...
def teacher_score(teachName_list,QTeach_list,score_list):
    name_teach_list = []
    for t1 in teachName_list:
        name_teach = "Dr_"+t1
        name_teach_list.append(name_teach)
        #print(name_teach)
    teacher_list = {t2 : [] for t2 in name_teach_list} # auto genarate list
    t4 = 0
    t5 = 0
    for indt,namet in enumerate(name_teach_list): # access each list in dict -> name_teach_list["Sayan"]
        #for t4 in range(len(teachNum_list)): # use number in list for number question of each teacher
        t4 = t4+QTeach_list[indt] 
        teacher_list[namet].append(score_list[t5:t4])
        #print(score_list[t5:t4])
        #print("t5:",t5,"|","t4:",t4)
        t5 = t5+QTeach_list[indt]
        #print(t3,"=",teacher_list[t3])
        correct_teach = sum(teacher_list[namet][0]) #access index with str for dict then access index with int for list
        wrong_teach =  QTeach_list[indt] - correct_teach
        teacher_list[namet].append(correct_teach)
        teacher_list[namet].append(wrong_teach)
        print(namet,"--->","✓:",correct_teach,"/",QTeach_list[indt],"|","✕:",wrong_teach,"/",QTeach_list[indt])
